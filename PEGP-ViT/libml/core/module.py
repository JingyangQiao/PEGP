...
class Module(ModuleABC):
    ...
    def __init__(self, cfg: ModuleConf):
        ...
        if self.cfg.num_epochs == 5:
            self.projection_epoch = 2
        else:
            self.projection_epoch = 29

        if self.cfg.pet_cls == "Adapter":
            self.alpha = 1
        elif self.cfg.pet_cls == "LoRA":
            if self.cfg.num_epochs == 5:
                self.alpha = 0.5
            else:
                self.alpha = 1
    ...
    def run(self, mode: str = "train"):
        ...
        #################################################################################################
        self.o_pets = copy.deepcopy(self.pets_.state_dict())
        self.pets.load_state_dict(self.o_pets)
        ...
        self.p_pets = copy.deepcopy(self.pets.state_dict())
        self.pets_.load_state_dict(self.p_pets)

        if self.current_task > 0:
            self.pets.load_state_dict(self.o_pets)
            self.pgp = False

            for i in range(len(self.pets_)):
                for k, (m, params) in enumerate(self.pets_[i].named_parameters()):
            ...
            self.q_pets = copy.deepcopy(self.pets.state_dict())

            ans = self.o_pets

            beta = 1.0 / (self.current_task + self.alpha)

            for k in self.o_pets.keys():
                ans[k] = self.p_pets[k] * (1 - beta) + self.q_pets[k] * beta
            self.pets_.load_state_dict(ans)
    ...
    def train_epoch(self, loader, num_batches):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            with self.trainer.no_backward_sync(self.model_wrap, skip_sync):
                with self.autocast():
                    if self.pgp:
                        ...
                        loss, loss_dict = self.train_step_(batch, batch_idx)
            ...
            if (batch_idx + 1) % na == 0:
                ...
                if self.current_task != 0 and self.current_epoch > self.projection_epoch and self.pgp:
                    for i in range(len(self.pets)):
                        for k, (m, params) in enumerate(self.pets[i].named_parameters()):
                            if m.endswith("layer.0.weight") and self.cfg.pet_cls == "Adapter":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i]["down"])
                            if m.endswith("layer.2.weight") and self.cfg.pet_cls == "Adapter":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i]["up"])
                            if m.endswith("lora_A.weight") and self.cfg.pet_cls == "LoRA":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i]["down"])
                            if m.endswith("lora_B.weight") and self.cfg.pet_cls == "LoRA":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i]["up"])
                            if m.endswith("key.tokens") and self.cfg.pet_cls == "Prefix":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i])
                            if m.endswith("tokens") and self.cfg.pet_cls == "Prompt":
                                params.grad.data = params.grad.data - torch.matmul(params.grad.data, self.feature_t[i])
    ...
    def evaluate(self, loader: DataLoader = None):
        ...
        self.pre_eval_epoch()
        result = self.eval_epoch(loader, train_loader)
