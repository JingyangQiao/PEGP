    ...
    def forward_features(self, x):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in [0, 1, 2, 3, 4]:
                if self.use_adapter:
                    self.collect[i] = {}
                    self.collect[i]["down"] = block.adapters.attn.down_
                    self.collect[i]["up"] = block.adapters.attn.up_
                if self.use_lora:
                    self.collect[i] = {}
                    self.collect[i]["down"] = block.attn.adapters.qkv.down_
                    self.collect[i]["up"] = block.attn.adapters.qkv.up_
                if self.use_prefix:
                    self.collect[i] = block.attn.prefix.key.act
                if self.use_prompt:
                    self.collect[i] = block.attn.prefix.act
