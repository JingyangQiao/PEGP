...
class LinearLoRA(nn.Module):
    def __init__(
        ...
        self.layer = nn.Sequential(self.lora_A, self.lora_B, self.scale)
    ...
    def forward(self, module: nn.Linear, input: torch.Tensor):
        ...
        # self.down_ = input
        # self.up_ = F.linear(input, self.lora_A)
        # output = F.linear(input, module.weight + weight)
        self.down_ = input
        self.up_ = self.layer[0](input)

        return module(input) + self.layer(input)
