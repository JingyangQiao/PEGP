...
from datasets.class_names import cifar100_classnames, imagenet100_classnames
...
class Ticlip(nn.Module):
    def __init__(self, args):
        ...
        self.image_prompt_pool = nn.ModuleList([
            nn.Linear(512, 5, bias=False).half(),
            nn.Linear(5, 512, bias=False).half(),
        ])  # Image Prompt Pool
        self.image_prompt_pool_ = nn.ModuleList([
            nn.Linear(512, 5, bias=False).half(),
            nn.Linear(5, 512, bias=False).half(),
        ])  # Image Prompt Pool
    ...
    def forward(self, image, mode=1):
        logits = []
        if mode == 1:
            image_features = self.image_prompt_pool[1](self.image_prompt_pool[0](self.image_encoder(image.type(self.dtype)))) + self.image_encoder(image.type(self.dtype))
        if mode == 2:
            image_features = self.image_prompt_pool_[1](self.image_prompt_pool_[0](self.image_encoder(image.type(self.dtype)))) + self.image_encoder(image.type(self.dtype))
    ...
    def interface(self, images, tasks):
        ...
        image_features = self.image_prompt_pool[1](self.image_prompt_pool[0](self.image_encoder(images.type(self.dtype)))) + self.image_encoder(images.type(self.dtype))
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
    ...
    def query(self, images):
        representations_down = self.image_encoder(images.type(self.dtype))
        representations_up = self.image_prompt_pool[0](self.image_encoder(images.type(self.dtype)))
        return representations_down, representations_up
