...
class TPrompts(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            ...
            for i in range(len(self.feature)):
                print("linear feature shape", self.feature[i].shape)
                self.feature_mat[i] = torch.Tensor(np.dot(self.feature[i], self.feature[i].transpose())).to(self._device)
                print('Linear Projection Matrix Shape: {}'.format(self.feature_mat[i].shape))
            ...
            self.o_prompt = copy.deepcopy(self._network.image_prompt_pool.state_dict())
            self.train_function(train_loader, test_loader, optimizer, schedule, feature_mat=self.feature_mat)
            self.p_prompt = copy.deepcopy(self._network.image_prompt_pool.state_dict())
            self._network.image_prompt_pool_.load_state_dict(self.o_prompt)
            for name, param in self._network.named_parameters():
                if "image_prompt_pool" in name:
                    param.requires_grad_(False)
                if "image_prompt_pool_" in name:
                ...
                # if "text_prompt_pool" + "." + str(self._network.task - 1) in name:
                #     param.requires_grad_(False)
            self.train_function_(train_loader, test_loader, optimizer, schedule)
            self.q_prompt = copy.deepcopy(self._network.image_prompt_pool_.state_dict())
            for name, param in self._network.named_parameters():
                if "image_prompt_pool" in name:
                    param.requires_grad_(True)
                if "image_prompt_pool_" in name:
            ...
            ans = self.o_prompt
            beta = 0.5
            for k in self.o_prompt.keys():
                ans[k] = self.p_prompt[k] * (1 - beta) + self.q_prompt[k] * beta
            self._network.image_prompt_pool.load_state_dict(ans)

        # Image Gradient Projection Matrix
        self._network.eval()
        mem_example = memory.get_representation_matrix(train_loader, self._device)
        rep0, rep1 = [], []

        for bs_ in range(16):
            rep_0, rep_1 = self._network.query(mem_example[bs_ * 32:(bs_ + 1) * 32, ...])
            rep_0, rep_1 = rep_0.reshape(32, -1), rep_1.reshape(32, -1)
            rep0.append(rep_0)
            rep1.append(rep_1)
        ...
        self._network.train()
        self.feature = memory.update_memory([rep0, rep1], 0.99, self.feature)
            ...
            for i, (tasks, inputs, targets) in enumerate(train_loader):
                ...
                if feature_mat is not None:
                    ...
                    for k, (m, params) in enumerate(self._network.named_parameters()):
                        if "image_prompt_pool" in m and "image_prompt_pool_" not in m:
                            params.grad.data = params.grad.data - torch.matmul(params.grad.data, feature_mat[count].half())
                            count += 1
    ...
    def train_function_(self, train_loader, test_loader, optimizer, schedule):
        bar = tqdm(range(5))
        for _, epoch in enumerate(bar):
            self._network.eval()
            losses = 0.
            ...
            for i, (tasks, inputs, targets) in enumerate(train_loader):
                ...
                inputs = torch.index_select(inputs, 0, mask)
                targets = torch.index_select(targets, 0, mask)
                logits = self._network(inputs, mode=2)
                loss = F.cross_entropy(logits, targets % 10)
                fa = self._network.extract(inputs, 1)
                fb = self._network.extract(inputs, 2)
