...
from datasets.data import iCifar100, ImageNet100, iImageNet100
...
class DataManager():
    ...
    def get_dataset(self, indices, mode, appendent=None, ret_data=False):
        ...
        elif mode == "train" and self.dataset_name == "imagenet100":
            dataset = ImageNet100(self.args["data_path"], train=True, download=True)
        elif mode == "test" and self.dataset_name == "imagenet100":
            dataset = ImageNet100(self.args["data_path"], train=False, download=True)
        else:
            raise ValueError("Unknown mode {}.".format(mode))
    ...
    def _select_img(self, dataset, low_range, high_range):
        x = []
        y = []
        ...
        for idx in range(len(data_path)):
            if high_range > int(target[idx]) >= low_range:
                x.append(Image.open(data_path[idx]).convert("RGB"))
                y.append(int(target[idx]))
        return np.array(x), np.array(y)
